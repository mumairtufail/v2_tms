    ...
    <nav class="flex-1 overflow-y-auto px-3 py-4 space-y-6 no-scrollbar h-[calc(100vh-160px)]">
        ...
        <div>
            ...
            <div class="space-y-1.5">
                ...
                <a href="{{ $currentCompany ? route('v2.activity-logs.index', ['company' => $companySlug]) : '#' }}"
                   class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 group {{ request()->routeIs('v2.activity-logs.*') ? $activeClasses : $inactiveClasses }}">
                    <div class="w-5 h-5 flex items-center justify-center {{ request()->routeIs('v2.activity-logs.*') ? $activeIconClasses : '' }}">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/>
                        </svg>
                    </div>
            ...
            <div class="space-y-1.5">
                ...
                <a href="{{ route('admin.logs') }}"
                   class="flex items-center gap-3 px-4 py-3 rounded-xl transition-all duration-300 group {{ request()->routeIs('admin.logs*') ? $activeClasses : $inactiveClasses }}">
                    ...
                    <span class="font-medium text-[14px]">All Activity Logs</span>
                </a>
            </div>
        </div>
        @endif
