...
<div class="space-y-5" x-data="orderForm()">
    ...
    @php
        $statusClasses = [
            ...
            'warehousing' => 'bg-purple-100 text-purple-700 border-purple-300',
            'picked_up' => 'bg-teal-100 text-teal-700 border-teal-300',
        ...
        <div class="flex items-center gap-2">
            ...
            @if($canChangeBooking && $order->status === 'quoted')
                <form method="POST" action="{{ route('v2.orders.book', ['company' => $company->slug, 'order' => $order->id]) }}">
                    @csrf
                    <x-secondary-button type="submit">Mark as Booked</x-secondary-button>
                </form>
            @elseif($canChangeBooking && $order->status === 'booked')
                <form method="POST" action="{{ route('v2.orders.unbook', ['company' => $company->slug, 'order' => $order->id]) }}">
                    @csrf
                    <x-secondary-button type="submit">Revert to Quoted</x-secondary-button>
                </form>
            @endif
            ...
            <x-primary-button @click="primaryAction()" type="button">
                <span x-text="isPortal ? 'Submit Order' : (isDraftOrder() ? 'Save & Submit' : (orderStatus === 'new' ? 'Submit to Quote' : 'Update Quote / Manifest'))"></span>
            </x-primary-button>
        </div>
    </div>
