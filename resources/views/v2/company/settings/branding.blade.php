...
@php
    ...
    $logoCards = [
        [
            ...
            'value' => $company->logo_light,
        ],
        [
            'key' => 'logo_dark',
            'label' => 'Dark Logo',
            'hint' => 'Used on dark surfaces and dark mode.',
            'value' => $company->logo_dark,
        ],
    ];
@endphp
...
<div class="space-y-6" x-data="brandingSettings()">
    ...
    <div class="flex flex-col gap-2">
        <x-page-header title="Branding & Logo" description="Upload separate brand assets for light mode, dark mode, and compact surfaces." />
        <p class="text-xs text-gray-500 dark:text-gray-400 max-w-3xl">
            Keep your identity consistent across the app with dedicated assets for each theme and layout.
        </p>
    </div>

    <form method="POST" action="{{ route('v2.settings.branding.update', $company) }}" enctype="multipart/form-data" @submit="submitting = true"
        ...
        <div class="flex items-center justify-between border-t border-gray-200 dark:border-gray-800 pt-4">
            <p class="text-xs text-gray-500 dark:text-gray-400">PNG, JPG, or SVG. Max 3MB for logo files and 2MB for the brand mark.</p>
            <button type="submit"
                ...
                <span x-show="submitting">
                    <x-loader size="xs" class="text-white" />
                </span>
...
function brandingSettings() {
    return {
        submitting: false,
        existing: {
            logo_light: @json((bool) $company->logo_light),
            logo_dark:  @json((bool) $company->logo_dark),
        },
        previews: {
            logo_light: null,
            logo_dark:  null,
