{{--
    24-Hour DateTime Picker  –  <x-datetime-picker x-model="someVar" @change="handler()" />
    Value in/out: "YYYY-MM-DDTHH:mm"  (drop-in for datetime-local)
    No AM/PM, no external libraries.
--}}
<div
    x-data="{
        value:   '',
        ...
        _parse(v) {
            if (!v) { this.dateVal = ''; this.selH = null; this.selM = null; return; }
            const parts = String(v).split('T');
            this.dateVal = parts[0] || '';
            if (parts[1]) {
                const t = parts[1].split(':');
                const h = parseInt(t[0]), m = parseInt(t[1] ?? 0);
        ...
        _scrollToSelected() {
            const itemH = 36;
            const hEl = this.$el.querySelector('.dtp-hour-scroll');
            const mEl = this.$el.querySelector('.dtp-min-scroll');
        ...
        _save() {
            ...
            const hh = this.selH !== null ? String(this.selH).padStart(2,'0') : '00';
            const mm = this.selM !== null ? String(this.selM).padStart(2,'0') : '00';
            this.value = this.dateVal + 'T' + hh + ':' + mm;
            this.$dispatch('change');
        },

        clear() {
            this.value = ''; this.dateVal = '';
            this.selH = null; this.selM = null;
            this.open = false;
            this.$dispatch('change');
        },
...
>
    ...
    <div class="flex items-center gap-1.5">
        ...
        {{-- Date – native input (no AM/PM issue with date) --}}
        <input type="date"
               x-model="dateVal"
               @change="onDateChange()"
               class="flex-1 min-w-0 text-sm px-2.5 py-1.5
        ...
        <button type="button" @click="open = !open"
                class="flex items-center gap-1.5 px-2.5 py-1.5 shrink-0
                       bg-white dark:bg-gray-800
                       border border-gray-200 dark:border-gray-700
                       rounded-lg hover:border-primary-400
                       focus:outline-none focus:ring-2 focus:ring-primary-500/30 focus:border-primary-500
            ...
            <span x-text="timeLabel" class="font-mono text-sm font-semibold tracking-widest whitespace-nowrap"
    ...
    <div x-show="open" x-cloak @click.outside="open = false"
         ...
         class="absolute z-50 top-full mt-1.5 right-0
                bg-white dark:bg-gray-800
                border border-gray-200 dark:border-gray-700
        ...
        <div class="flex">
            {{-- Hours 00–23 --}}
            <div class="dtp-hour-scroll flex-1 h-52 overflow-y-auto overscroll-contain py-1">
                @for ($h = 0; $h < 24; $h++)
            ...
            <div class="dtp-min-scroll flex-1 h-52 overflow-y-auto overscroll-contain py-1">
                @for ($m = 0; $m < 60; $m += 5)
