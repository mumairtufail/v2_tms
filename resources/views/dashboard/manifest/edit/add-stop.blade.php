...
<div class="modal fade add-stop-modal" id="addStopModal" tabindex="-1">
    <div class="modal-dialog modal-lg">
        <form id="stopForm" class="stop-form" action="{{ route('stops.createStop') }}" method="POST">
            ...
            <input type="hidden" name="_method" id="stopMethod" value="POST">
            <input type="hidden" name="stop_id" id="stopId">
...
<script>
    ...
    function editStop(stop) {
        // Set form fields
        const form = document.getElementById('stopForm');
        form.action = `/stops/${stop.id}`;
        document.getElementById('stopMethod').value = 'PUT';
        document.getElementById('stopId').value = stop.id;
        
        // Fill form fields
        form.querySelector('[name="company"]').value = stop.company;
        form.querySelector('[name="address1"]').value = stop.address1;
        form.querySelector('[name="address2"]').value = stop.address2;
        form.querySelector('[name="city"]').value = stop.city;
        form.querySelector('[name="state"]').value = stop.state;
        form.querySelector('[name="country"]').value = stop.country;
        form.querySelector('[name="postal"]').value = stop.postal;
        form.querySelector('[name="location"]').value = stop.location;
    ...
    function confirmDeleteStop(stopId) {
        Swal.fire({
            title: 'Are you sure?',
            text: "This stop will be permanently deleted!",
            icon: 'warning',
    ...
    function deleteStop(stopId) {
        ...
        .then(data => {
            if (data.success) {
                ...
                Swal.fire('Error!', 'Could not delete stop.', 'error');
            }
        });
    }
</script>